...
def test_net(cfg,
        ...
        with torch.no_grad():
            ...
            if output_dir and sample_idx < 3:
                ...
                rendering_views = utils.binvox_visualization.get_volume_views(gv, os.path.join(img_dir, 'test'),
                                                                              epoch_idx, 'gv')
                test_writer.add_image('Epoch#%02d/Test Sample#%02d/Volume Reconstructed' % (epoch_idx, sample_idx), rendering_views.transpose(2, 0, 1), epoch_idx)
                gtv = ground_truth_volume.cpu().numpy()
                rendering_views = utils.binvox_visualization.get_volume_views(gtv, os.path.join(img_dir, 'test'),
                                                                              epoch_idx, 'gtv')
                test_writer.add_image('Epoch#%02d/Test Sample#%02d/Volume GroundTruth' % (epoch_idx, sample_idx), rendering_views.transpose(2, 0, 1), epoch_idx)
    ...
    for taxonomy_id in test_iou:
        ...
        viewKey = '%d-view' % cfg.CONST.N_VIEWS_RENDERING
        if 'baseline' in taxonomies[taxonomy_id] and viewKey in taxonomies[taxonomy_id]['baseline']:
            print('%.4f' % taxonomies[taxonomy_id]['baseline'][viewKey], end='\t\t')
        else:
            print('N/a', end='\t\t')
